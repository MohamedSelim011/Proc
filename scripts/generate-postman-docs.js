...
const BASE_URL = 'https://bright-procurement-production.up.railway.app';
...
function routePathFromFile(filePath) {
  ...
  const raw = relDir === '' ? '/api' : `/api/${relDir}`;
...
function needsAuthHeader(block, routePath) {
  if (routePath === '/api/auth/signin') return false;
  if (routePath.includes('/submit/{{')) return false;
  if (routePath.includes('/vendor-response/{{')) return false;
  if (routePath.includes('/acknowledge')) return false;
  if (/authorization|bearer|verifytoken|gettoken|jwt|session/i.test(block)) return true;
  return true;
}
...
function generateDocs(endpoints) {
  const lines = [];
  lines.push('# Bright Procurement API - Postman Documentation');
  lines.push('');
  lines.push(`Base URL: ${BASE_URL}`);
  lines.push('');
  lines.push('Notes:');
  lines.push('- This documentation is generated by scanning `src/app/api/**/route.ts`.');
...
function main() {
  ...
  const postmanCollection = {
    info: {
      name: 'Bright Procurement API',
      _postman_id: 'a8ee5c16-927b-4f55-87f8-bcbf22689337',
      description: 'Auto-generated from src/app/api route handlers.',
  ...
  const postmanEnvironment = {
    id: '8cb3976f-17d2-44fc-a008-6224e2a75627',
    name: 'Bright Procurement Environment',
    values: [
      { key: 'baseUrl', value: BASE_URL, enabled: true },
      { key: 'token', value: 'paste-jwt-token-here', enabled: true },
    ],
  ...
  fs.writeFileSync(path.join(OUT_DIR, 'bright-procurement.postman_collection.json'), JSON.stringify(postmanCollection, null, 2));
  fs.writeFileSync(path.join(OUT_DIR, 'bright-procurement.postman_environment.json'), JSON.stringify(postmanEnvironment, null, 2));

  const md = generateDocs(endpoints);
  fs.writeFileSync(path.join(OUT_DIR, 'API_DOCUMENTATION.md'), md);

  console.log(`Generated endpoints: ${endpoints.length}`);
  console.log('Collection: docs/postman/bright-procurement.postman_collection.json');
  console.log('Environment: docs/postman/bright-procurement.postman_environment.json');
  console.log('Markdown: docs/postman/API_DOCUMENTATION.md');
}
