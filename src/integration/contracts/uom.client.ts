export type UomIntegrationQuery = {
  search?: string;
  type?: string;
  isActive?: string;
};
...
function buildListUrl(query: UomIntegrationQuery) {
  const url = new URL('/procurement/uom', getIntegrationBaseUrl());

  if (query.search) url.searchParams.set('search', query.search);
  if (query.type) url.searchParams.set('type', query.type);
  if (query.isActive) url.searchParams.set('isActive', query.isActive);

  return url.toString();
}

export async function fetchUomFromIntegration(
  query: UomIntegrationQuery,
  ...
  if (!response.ok) {
    ...
    const error = new Error('Failed to fetch units of measure from integration middleware.') as Error & {
      status?: number;
      details?: unknown;
    };
