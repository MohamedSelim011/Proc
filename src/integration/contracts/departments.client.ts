export type DepartmentsIntegrationQuery = {
  search?: string;
  type?: string;
};
...
function buildListUrl(query: DepartmentsIntegrationQuery) {
  const url = new URL('/procurement/departments', getIntegrationBaseUrl());
...
export async function fetchDepartmentsFromIntegration(
  query: DepartmentsIntegrationQuery,
  ...
  if (!response.ok) {
    ...
    const error = new Error('Failed to fetch departments from integration middleware.') as Error & {
      status?: number;
      details?: unknown;
    };
