...
import { ArrowLeft, Building2, Calendar, Hash, Loader2, Pencil, Trash2 } from 'lucide-react';
  ...
  useEffect(() => {
    const run = async () => {
      try {
        ...
        const payload = (await response.json()) as { data?: { departmentsIntegrationEnabled?: boolean } };
    ...
    <div className="space-y-6">
      <div className="rounded-xl border border-gray-200 bg-white p-6 shadow-sm">
        <div className="flex flex-wrap items-start justify-between gap-4">
          <div>
            <p className="text-xs font-semibold uppercase tracking-wide text-gray-500">Department Details</p>
            <h1 className="mt-1 text-2xl font-bold text-gray-900">{item.name}</h1>
            <p className="mt-1 text-sm text-gray-600">
              Code: <span className="font-medium text-gray-800">{displayOrDash(item.code)}</span>
            </p>
          </div>
          <div className="flex items-center gap-2">
            ...
            {departmentsIntegrationEnabled === false ? (
              <>
                <Link
                  href={`/procurement/organization/departments/${item.id}/edit`}
                ...
                <button
                  ...
                  onClick={async () => {
                    ...
                    try {
                      setDeleting(true);
                      const response = await apiFetch(`/api/organization/departments/${item.id}`, { method: 'DELETE' });
                      if (!response.ok) {
                        const payload = (await response.json().catch(() => ({}))) as { error?: string };
                        alert(payload.error || 'Failed to delete department');
                        return;
                      }
                      router.push('/procurement/organization/departments');
                      router.refresh();
                    } finally {
                      setDeleting(false);
                    }
      ...
      <div className="grid grid-cols-1 gap-4 md:grid-cols-4">
        ...
        <div className="rounded-xl border border-gray-200 bg-white p-4 shadow-sm">
          ...
          <p className="mt-2 text-lg font-semibold text-gray-900">{displayOrDash(item.type)}</p>
        </div>
        <div className="rounded-xl border border-gray-200 bg-white p-4 shadow-sm">
          <p className="text-xs font-medium uppercase tracking-wide text-gray-500">Cost Center</p>
          <p className="mt-2 text-lg font-semibold text-gray-900">{displayOrDash(item.costCenterCode)}</p>
        </div>
        <div className="rounded-xl border border-gray-200 bg-white p-4 shadow-sm">
          <p className="text-xs font-medium uppercase tracking-wide text-gray-500">Integration Mode</p>
          <p className="mt-2 text-sm font-semibold text-gray-900">
            {departmentsIntegrationEnabled ? 'EXTERNAL SYNC ENABLED' : 'INTERNAL MANAGEMENT'}
          </p>
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 gap-6 xl:grid-cols-2">
        <div className="rounded-xl border border-gray-200 bg-white p-5 shadow-sm">
          <h2 className="flex items-center gap-2 text-sm font-semibold text-gray-900">
            <Building2 className="h-4 w-4 text-wujha-primary" />
            General Information
          </h2>
          <dl className="mt-3 space-y-2 text-sm">
            <div className="flex justify-between gap-4">
              <dt className="text-gray-500">Department Name</dt>
              <dd className="text-gray-900">{displayOrDash(item.name)}</dd>
            </div>
            <div className="flex justify-between gap-4">
              <dt className="text-gray-500">Department Code</dt>
              <dd className="text-gray-900">{displayOrDash(item.code)}</dd>
            </div>
            <div className="flex justify-between gap-4">
              <dt className="text-gray-500">Department Type</dt>
              <dd className="text-gray-900">{displayOrDash(item.type)}</dd>
            </div>
            <div className="flex justify-between gap-4">
              <dt className="text-gray-500">Cost Center Code</dt>
              <dd className="text-gray-900">{displayOrDash(item.costCenterCode)}</dd>
            </div>
          </dl>
        </div>
        ...
        <div className="rounded-xl border border-gray-200 bg-white p-5 shadow-sm">
          ...
          <dl className="mt-3 space-y-2 text-sm">
            ...
            <div className="flex justify-between gap-4">
              ...
              <dd className="max-w-[60%] break-all text-right text-gray-900">{displayOrDash(item.externalId)}</dd>
            </div>
            <div className="flex justify-between gap-4">
              <dt className="text-gray-500">Parent External ID</dt>
              <dd className="max-w-[60%] break-all text-right text-gray-900">{displayOrDash(item.parentExternalId)}</dd>
            </div>
          </dl>
        </div>
      </div>
      ...
      <div className="rounded-xl border border-gray-200 bg-white p-5 shadow-sm">
        ...
        <div className="mt-3 rounded-md border border-gray-200 bg-gray-50 p-4 text-sm text-gray-700">
          {displayOrDash(item.description)}
        </div>
      </div>
      ...
      <div className="rounded-xl border border-gray-200 bg-white p-5 shadow-sm">
        ...
        <div className="mt-4 grid grid-cols-1 gap-3 md:grid-cols-3 text-sm text-gray-700">
          <p>
