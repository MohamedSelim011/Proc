...
export async function POST(request: NextRequest) {
  try {
    ...
    if (!response.ok) {
      ...
      console.error('Reporting engine preview with columns error:', {
    ...
    console.error('Error previewing report with columns from reporting engine:', error);
    return NextResponse.json(
      { error: 'Failed to preview report with columns from reporting engine' },
      { status: 500 }
    );
  }
}
