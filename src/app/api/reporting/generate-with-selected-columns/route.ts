...
export async function POST(request: NextRequest) {
  try {
    ...
    const response = await fetch(`${REPORTING_ENGINE_URL}/api/v1/reports/generate-with-selected-columns`, {
      method: 'POST',
      headers: {
    ...
    if (!response.ok) {
      ...
      console.error('Reporting engine generate with selected columns error:', {
        status: response.status,
        statusText: response.statusText,
        response: errorText,
        url: `${REPORTING_ENGINE_URL}/api/v1/reports/generate-with-selected-columns`
      });
    ...
    console.error('Error generating report with selected columns from reporting engine:', error);
    return NextResponse.json(
      { error: 'Failed to generate report with selected columns from reporting engine' },
      { status: 500 }
    );
  }
}
