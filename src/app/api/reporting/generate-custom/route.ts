...
export async function POST(request: NextRequest) {
  try {
    ...
    const response = await fetch(`${REPORTING_ENGINE_URL}/api/v1/reports/generate-custom?tableName=${encodeURIComponent(tableName)}${whereClause ? `&whereClause=${encodeURIComponent(whereClause)}` : ''}`, {
      method: 'POST',
      headers: {
    ...
    if (!response.ok) {
      ...
      console.error('Reporting engine generate custom error:', {
        status: response.status,
        statusText: response.statusText,
        response: errorText,
        url: `${REPORTING_ENGINE_URL}/api/v1/reports/generate-custom`
      });
    ...
    if (response.headers.get('content-type')?.includes('application/pdf')) {
      ...
      return new NextResponse(buffer, {
        ...
        headers: {
          ...
          'Content-Disposition': `attachment; filename="${tableName}_custom_report.pdf"`
        }
      });
    }
    ...
    console.error('Error generating custom report from reporting engine:', error);
    return NextResponse.json(
      { error: 'Failed to generate custom report from reporting engine' },
      { status: 500 }
    );
  }
}
