...
// GET /api/service-contracts/[id]/download - Download Service Contract as PDF
export async function GET(
  ...
  try {
    ...
    // Fetch the service contract with all related data
    ...
    const formatJsonField = (value: string | object | null | undefined): string => {
      ...
      // If it's an array, format as a bulleted list
      if (Array.isArray(value)) {
        if (value.length === 0) return '';
        return value.map((item, index) => {
          if (typeof item === 'object' && item !== null) {
            // If array item is an object, format its properties
      ...
      // If it's an object, format as readable key-value pairs
        ...
        <body>
          <div class="header">
            ...
            <div class="header-right">
              ...
              <div class="document-meta">
                ...
                <div><strong>Status:</strong> <span class="status-badge status-${contract.status.toLowerCase()}">${contract.status}</span></div>
              </div>
            </div>
          </div>
            ...
            <div class="info-grid">
              <div class="info-column">
                ...
                <div class="info-item">
                  ...
                  <div style="font-size: 11px; color: #6b7280; margin-top: 4px;">${contract.vendor.vendorCode}</div>
                </div>
              </div>
              <div class="info-column">
                <div class="info-item">
              ...
              ${contract.performanceBond ? `
                <div class="info-column">
                  <div class="info-item">
                    ...
                    <div class="info-value">${formatCurrency(Number(contract.performanceBond), contract.currency)}</div>
                  </div>
                </div>
              ` : ''}
              ${contract.retentionAmount ? `
                <div class="info-column">
                  <div class="info-item">
                    ...
                    <div class="info-value">${formatCurrency(Number(contract.retentionAmount), contract.currency)}</div>
                  </div>
                </div>
              ` : ''}
            </div>
          ...
          <div class="section">
            ...
            ${contract.slaTerms ? `
              <div class="terms-box">
                <h4>SLA Terms</h4>
                <p style="white-space: pre-line; margin: 0;">${formatJsonField(contract.slaTerms)}</p>
              </div>
            ` : ''}
            ${contract.penaltyClause ? `
              <div class="terms-box" style="background: #fee2e2; border-color: #ef4444;">
                ...
                <p style="color: #991b1b; margin: 0;">${contract.penaltyClause}</p>
              </div>
            ` : ''}
            ${contract.insuranceRequirements ? `
              <div class="terms-box">
                <h4>Insurance Requirements</h4>
                <p style="white-space: pre-line; margin: 0;">${formatJsonField(contract.insuranceRequirements)}</p>
              </div>
            ` : ''}
          </div>
          ` : ''}
