              ...
              <tbody>
                ${version.contract.servicePR.materialItems.map((item: any) => `
                  <tr>
                    ...
                    <td>${formatCurrency(Number(item.estimatedPrice || 0), version.currency)}</td>
                    <td>${formatCurrency(Number(item.quantity || 0) * Number(item.estimatedPrice || 0), version.currency)}</td>
                  </tr>
                `).join('')}
              </tbody>
            </table>
